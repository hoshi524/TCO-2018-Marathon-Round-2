...
public class CrystalLightingVis {
        ...
        try {
            ...
            for (int r = 0; r < H; ++r)
                for (int c = 0; c < W; ++c) {
                    ...
                    targetBoard[r][c] = (t < pCrystal ? (char) (r1.nextInt(6) + 1 + '0')
                            : (t < pCrystal + pObstacle ? 'X' : '.'));
                    if (t < pCrystal)
                        numCrystals++;
                }
    ...
    String placeOneItem(int R, int C, char type) {
        ...
        if (getTypeIdx(type) < 0) {
            return "Invalid item type: " + type
                    + ". You can only place lanterns of primary colors (1, 2 or 4), mirrors (\\ or /) or obstacles (X).";
        }
        if (targetBoard[R][C] != '.') {
    ...
    int getScore() {
        ...
        // Score is: 20 for each correct primary color crystal, 30 for each correct
        ...
        if (debug) {
            System.out.println(numCorrectPrimary + " " + (20 * numCorrectPrimary));
            System.out.println(numCorrectSecondary + " " + (30 * numCorrectSecondary));
            System.out.println(numIncorrect + " " + (-10 * numIncorrect));
            System.out.println(addedItems.size() + " " + -costLantern * addedItems.size());
        }
        return score;
    }
    ...
    public int runTest(String seed) {
        try {
            ...
            if (proc != null) {
                ...
                String[] itemsRet;
                try {
                    itemsRet = placeItems(targetBoardArg, costLantern, costMirror, costObstacle, maxMirrors,
                            maxObstacles);
                } catch (Exception e) {
                ...
                // pure manual mode just returns 0 items to process, because they will be placed
                // by the player
                for (int i = 0; i < itemsRet.length; ++i) {
                    ...
                    if (s.length != 3) {
                        addFatalError(
                                "Item " + i + ": Each element of your return must be formatted as \"ROW COL TYPE\"");
                        return invalidScore;
                    }
                    int R, C;
                    try {
                    ...
                    if (s[2].length() != 1) {
                        addFatalError("Item " + i + ": Invalid item type: " + s[2]
                                + ". Item type must be a single character.");
                        return invalidScore;
                    }
    ...
    // -----------------------------------------
    String[] placeItems(String[] targetBoard, int costLantern, int costMirror, int costObstacle, int maxMirrors,
            int maxObstacles) throws IOException {
        StringBuffer sb = new StringBuffer();
        sb.append(H).append("\n");
        for (int i = 0; i < H; ++i) {
    ...
    public class Vis extends JPanel implements MouseListener, WindowListener {
        public void paint(Graphics g) {
            ...
            if (showRays) {
                ...
                for (Ray ray : rays) {
                    ...
                    g2.drawLine(ray.c0 * SZ + SZ / 2, ray.r0 * SZ + SZ / 2, Math.min(W * SZ, ray.c1 * SZ + SZ / 2),
                            Math.min(H * SZ, ray.r1 * SZ + SZ / 2));
                }
            }
            ...
            for (int r = 0; r < H; ++r)
                for (int c = 0; c < W; ++c) {
                    if (targetBoard[r][c] != '.' && targetBoard[r][c] != 'X' && resultBoard[r][c] != '.') {
                        ...
                        g2.fillPolygon(
                                new int[] { c0 - SZ / 3, c0, c0 + SZ / 3, c0 + SZ / 6, c0 - SZ / 6, c0 - SZ / 3 },
            ...
            for (int i = 0; i < addedItems.size(); ++i) {
                ...
                if (item.type == 'X') {
                    ...
                    g2.drawLine(item.c * SZ + SZ / 3, item.r * SZ + SZ / 3, item.c * SZ + 2 * SZ / 3,
                            item.r * SZ + 2 * SZ / 3);
                    g2.drawLine(item.c * SZ + 2 * SZ / 3, item.r * SZ + SZ / 3, item.c * SZ + SZ / 3,
                            item.r * SZ + 2 * SZ / 3);
                } else if (item.type == '\\') {
            ...
            // finally, the required colors of the crystals (shown as color of their
            // outline)
            for (int r = 0; r < H; ++r)
                for (int c = 0; c < W; ++c) {
                    if (targetBoard[r][c] == '.')
        ...
        public void mousePressed(MouseEvent e) {
            ...
            if (!isInside(row, col))
                return;
            // ignore clicks on obstacles and crystals (can't place an item there or remove
            // it)
                ...
                for (int i = 0; i < addedItems.size(); ++i) {
                    ...
                    if (item.r == row && item.c == col) {
                        ...
                        while (true) {
                            ...
                            if (type == 'X' && usedObstacles >= maxObstacles)
                                continue; // All obstacles used
                            if ((type == '\\' || type == '/') && item.type != '\\' && item.type != '/'
                                    && usedMirrors >= maxMirrors)
